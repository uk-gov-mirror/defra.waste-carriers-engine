...
module WasteCarriersEngine
  RSpec.describe CompanyAddressManualForm do
    describe "#initialize" do
      context "when the transient registration has an address already" do
        let(:registered_address) { build(:address, :registered, :has_required_data) }
        ...
        let(:company_address_manual_form) { described_class.new(transient_registration) }
        ...
        context "when the business is overseas" do
          ...
          it "prefills the form with the existing address" do
            expect(company_address_manual_form.house_number).to eq(registered_address.house_number)
          end
        end
        ...
        context "when the temp_company_postcode matches the existing address" do
          before do
            transient_registration.temp_company_postcode = registered_address.postcode
          end
        ...
        context "when the temp_company_postcode is in use and doesn't match the registered address" do
          ...
          it "does not prefill the form with the existing address" do
            expect(company_address_manual_form.house_number).not_to eq(registered_address.address_line_1)
          end
        end
      end
    end
    ...
    describe "#submit" do
      context "when the form is valid" do
        ...
        it "submits" do
          expect(company_address_manual_form.submit(valid_params)).to be true
        end
      end
      ...
      context "when the form is not valid" do
        ...
        it "does not submit" do
          expect(company_address_manual_form.submit(invalid_params)).to be false
        end
      end
    end
    ...
    context "when a valid company address exists and is still valid" do
      ...
      let(:transient_registration) do
        build(
          ...
          workflow_state: "company_address_manual_form",
          registered_address: registered_address,
          temp_company_postcode: registered_address.postcode
        )
      end
      # Don't use FactoryBot for this as we need to make sure it initializes with a specific object
      let(:company_address_manual_form) { described_class.new(transient_registration) }
      ...
      describe "#house_number" do
        context "when the house_number is blank" do
          before do
            transient_registration.registered_address.house_number = nil
          end

          it "is not valid" do
            expect(company_address_manual_form).not_to be_valid
          end
        end
      ...
      describe "#address_line_1" do
        context "when the address_line_1 is blank" do
          before do
            transient_registration.registered_address.address_line_1 = nil
          end
      ...
      describe "#town_city" do
        context "when the town_city is blank" do
          before do
            transient_registration.registered_address.town_city = nil
          end
        ...
        context "when the town_city is too long" do
          before do
            transient_registration.registered_address.town_city = "4jhjdq46425oqers8r0b0xejkl19bapc"
          end
      ...
      describe "#postcode" do
        context "when the postcode is too long" do
          before do
            transient_registration.registered_address.postcode = "4jhjdq46425oqers8r0b0xejkl19bapc"
          end
      ...
      describe "#country" do
        context "when the country is blank" do
          before do
            transient_registration.registered_address.country = nil
          end
        ...
        context "when the country is too long" do
          before do
            transient_registration.registered_address.country = "f8x4jhjdq46425oqers8r0b0xejkl19bapc4jhjdq46425oqers"
          end
